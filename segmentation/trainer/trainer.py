...
from segmentation.base import BaseTrainer
from segmentation.utils import WarmupPolyLR, runningScore, cal_text_score
...
class Trainer(BaseTrainer):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            if self.tensorboard_enable and self.config['local_rank'] == 0:
                ...
                for key, value in loss_dict.items():
                    self.writer.add_scalar('TRAIN/LOSS/{}'.format(key), value, epoch)
                self.writer.add_scalar('TRAIN/ACC_IOU/acc', acc, epoch)
                self.writer.add_scalar('TRAIN/ACC_IOU/iou_shrink_map', iou_shrink_map, epoch)
                self.writer.add_scalar('TRAIN/lr', lr, epoch)
                if self.global_step % self.show_images_iter == 0:
                    # show images on tensorboard
                    self.inverse_normalize(batch['img'])
                    self.writer.add_images('TRAIN/imgs', batch['img'], epoch)
                    ...
                    self.writer.add_image('TRAIN/gt', show_label, self.epochs)
                    # model output
                    show_pred = []
                    ...
                    self.writer.add_image('TRAIN/preds', show_pred, self.epochs)
    ...
    def _on_epoch_finish(self):
        ...
        if self.config['local_rank'] == 0:
            ...
            if self.validate_loader is not None and self.metric_cls is not None:  # 使用f1作为最优模型指标
                ...
                # Lưu lại lịch sử
                self.history["epoch"].append(self.epoch_result['epoch'])
                self.history["train_loss"].append(self.epoch_result['train_loss'])
                
                if self.validate_loader is not None:
                   # nếu bạn có val_loss riêng thì log, nếu không thì có thể gán = train_loss
                   self.history["val_loss"].append(self.epoch_result['train_loss'])
                   self.history["precision"].append(precision)
                   self.history["recall"].append(recall)
                   self.history["hmean"].append(hmean)
                
                if self.tensorboard_enable:
                    self.writer.add_scalar('EVAL/recall', recall, self.epoch_result['epoch'])
                    self.writer.add_scalar('EVAL/precision', precision, self.epoch_result['epoch'])
                    self.writer.add_scalar('EVAL/hmean', hmean, self.epoch_result['epoch'])
    ...
    def plot_history(self, save_path="results.png", max_xticks=5):
        ...
        # Tính khoảng cách giữa các tick để chỉ hiện max_xticks thôi
        if len(epochs) > max_xticks:
            step = max(1, len(epochs) // max_xticks)
            xticks = np.arange(0, len(epochs), step)
            xticklabels = [epochs[i] for i in xticks]
        else:
        ...
        # Hmean
        plt.subplot(2, 2, 4)
        if len(self.history["hmean"]) > 0:
            plt.plot(epochs, self.history["hmean"], label="Hmean")
            plt.legend(); plt.title("Hmean")
            plt.xticks(xticks, xticklabels)
    ...
    def _on_train_finish(self):
        ...
        self.plot_history(f"{self.checkpoint_dir}/results.png")
